pipeline {
    ...
    parameters {
        ...
        choice(
            name: 'TEST_SUITE',
            choices: ['all', 'smoke', 'regression', 'api', 'e2e'],
    ...
    stages {
        ...
        stage('🎮 Test Environment Setup') {
            ...
            steps {
                script {
                    echo '🚀 Starting local test applications'
                    sh '''
        ...
        stage('📊 Quality Gates') {
            steps {
                script {
                    ...
                    ├── Passed: ${passedTests}
                    ├── Failed: ${failedTests}
                    └── Pass Rate: ${passRate.round(2)}%
                    """
                    
                    // Quality gate checks
                    if (passRate < 95 && env.BRANCH_NAME == 'main') {
                        error("❌ Quality gate failed: Pass rate ${passRate.round(2)}% is below 95% threshold for main branch")
                    }
                    
                    if (failedTests > 0 && params.TEST_SUITE == 'smoke') {
                        error("❌ Quality gate failed: Smoke tests must have 100% pass rate")
                    }
    ...
    post {
        ...
        success {
            script {
                ...
                slackSend(
                    ...
                    📈 *Reports:*
                    • <${env.BUILD_URL}Playwright_20Test_20Report|Test Report>
        ...
        failure {
            script {
                ...
                slackSend(
                    ...
                    • <${env.BUILD_URL}Playwright_20Test_20Report|Test Report>
                    • <${env.BUILD_URL}artifact|Test Artifacts>
        ...
        unstable {
            echo '⚠️ Build unstable - some tests failed but build continued'
